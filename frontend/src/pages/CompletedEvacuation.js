...
function CompletedEvacuation() {
  ...
  useEffect(() => {
    const fetchEvacuations = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/evacuation/completed', {
          headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-2xl font-semibold text-gray-900 mb-6">Completed Evacuations</h1>
        
        {evacuations.length === 0 ? (
          <div className="text-center text-gray-500">No completed evacuations found.</div>
        ) : (
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-500">
                          {new Date(evac.updated_at).toLocaleDateString()}
                        </p>
                      </div>
...
export default CompletedEvacuation; 