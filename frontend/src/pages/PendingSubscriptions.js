...
function PendingSubscriptions() {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/applications/pending-subscriptions', {
          headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-2xl font-semibold text-gray-900 mb-6">Bekleyen Abonelikler</h1>
        
        {applications.length === 0 ? (
          <div className="text-center text-gray-500">Bekleyen abonelik bulunamadı.</div>
        ) : (
